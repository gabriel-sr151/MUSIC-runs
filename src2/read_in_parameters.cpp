...
InitData read_in_parameters(std::string input_file) {
    ...
    double temp_homogeneous_e0 = 5.0; //Gev/fm3
    tempinput = Util::StringFind4(input_file, "homogeneous_e0");
    if(tempinput != "empty") istringstream ( tempinput ) >> temp_homogeneous_e0;
    parameter_list.homogeneous_e0 = temp_homogeneous_e0;

    double temp_homogeneous_vz = 0.5;
    tempinput = Util::StringFind4(input_file, "homogeneous_vz");
    if(tempinput != "empty") istringstream ( tempinput ) >> temp_homogeneous_vz;
    parameter_list.homogeneous_vz = temp_homogeneous_vz;

    double temp_homogeneous_bulk0 = 3.0; //Gev/fm3
    tempinput = Util::StringFind4(input_file, "homogeneous_bulk0");
    if(tempinput != "empty") istringstream ( tempinput ) >> temp_homogeneous_bulk0;
    parameter_list.homogeneous_bulk0 = temp_homogeneous_bulk0;
...
void check_parameters(InitData &parameter_list, std::string input_file) {
    ...
    if (parameter_list.turn_on_bulk == 0 
            && parameter_list.Initial_profile == 555) {
        music_message << "Profile initialized with bulk, but bulk is turned off CONTRADICTION!!";
        music_message.flush("warning");
        exit(1);
    }

    if (parameter_list.viscosity_flag == 0 
            && parameter_list.Initial_profile == 555) {
        music_message << "Profile initialized with bulk, but the viscosity flag is turned off CONTRADICTION!!";
        music_message.flush("warning");
        exit(1);
    }
