...
void Init::InitArena(SCGrid &arena_prev, SCGrid &arena_current,
    ...
    if (DATA.Initial_profile == 0) {
        ...
        music_message << "dx = " << DATA.delta_x << ", dy = " << DATA.delta_y
                      << ", deta = " << DATA.delta_eta;
        music_message.flush("info");
    } else if (DATA.Initial_profile == 555)
    {
        music_message << "Using Initial_profile = " << DATA.Initial_profile;
        music_message.flush("info");
        DATA.nx = 2;
...
void Init::InitTJb(SCGrid &arena_prev, SCGrid &arena_current) {
    ...
    {
        music_message.info(" ----- performing 1+1 D run ... -----");
        initial_homogeneous_1D(arena_prev, arena_current);
    }
...
void Init::initial_homogeneous_1D(SCGrid &arena_prev, SCGrid &arena_current) {
    ...
    for (int ieta = 0; ieta < neta; ieta++) {
        double rhob = 0.0;
        double vz = DATA.homogeneous_vz;
        double gamma = 1.0/sqrt(1-vz*vz); 
        double epsilon = DATA.homogeneous_e0/hbarc;   // fm^-4
        for (int ix = 0; ix < nx; ix++) {
            for (int iy = 0; iy< ny; iy++) {
                // set all values in the grid element:
                arena_current(ix, iy, ieta).epsilon = epsilon;
                ...
                arena_current(ix, iy, ieta).u[1] = 0.0;
                arena_current(ix, iy, ieta).u[2] = 0.0;
                arena_current(ix, iy, ieta).u[3] = gamma*vz;

                arena_current(ix, iy, ieta).pi_b = DATA.homogeneous_bulk0/hbarc;
