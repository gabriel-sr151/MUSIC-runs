...
class GridT {
 ...
 public:
    ...
    //    if(eta<0)   eta=0;  else if(eta>=Neta) eta= Neta - 1; GSR changed to periodic bdry condition
        if(eta==-2) eta = Neta-2; else if(eta==Neta-1); else if(eta==Neta) eta = 0; else if(eta==Neta+1) eta = 1;
